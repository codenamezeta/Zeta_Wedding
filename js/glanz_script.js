;(function ($) {
  ...
  if ($('#instagram-carousel').length > 0) {
    var feed = new Instafeed({
      ...
      sortBy: 'most-liked',
      template:
        '<div class="gla_bordered_block gla_image_bcked_zoom"><a href="{{link}}" target="_blank"></a><div class="gla_image_over gla_image_bck" data-image="{{image}}"></div><div class="gla_box_content text-center"><div class="gla_bottom_title gla_hidden_title"><p>{{caption}}</p></div></div></div>',
  ...
  $('.gla_page').each(function () {
    ...
    if (border) {
      $(
        '.gla_border_top, .gla_border_bottom, .gla_border_left, .gla_border_right, .gla_sml_abs_title'
      ).css('background', border)
      $('.gla_bordered_block').css('border-left-color', border)
      $('.gla_border')
        .css('border-bottom-color', border)
        .css('border-top-color', border)
